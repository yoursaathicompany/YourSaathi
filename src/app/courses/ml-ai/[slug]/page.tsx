...
export default async function CourseModulePage({ params }: Props) {
  ...
  const currentIdx = courseModules.findIndex((m) => m.slug === slug);
  ...
  const gradient = gradientColors[mod.number] || 'from-purple-500 to-pink-500';
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .mod-fade-up { animation: mod-fade-up 0.6s ease-out forwards; }
        .mod-float   { animation: mod-float 4s ease-in-out infinite; }
        .theory-card { 
      ...
      <div className="min-h-screen pb-20">
        ...
        <section className="relative overflow-hidden pt-20 pb-14 sm:pt-24 sm:pb-16">
          <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
            <div className="absolute top-0 left-1/3 w-[500px] h-[350px] bg-purple-600/8 rounded-full blur-[120px]" />
            <div className="absolute bottom-0 right-1/4 w-[400px] h-[300px] bg-pink-600/6 rounded-full blur-[100px]" />
          </div>
          ...
          <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex items-start gap-5 mb-6">
              <span className="text-5xl sm:text-6xl mod-float select-none flex-shrink-0">
                {mod.emoji}
              </span>
              <div>
                ...
                <h1 className="text-3xl sm:text-4xl font-black text-white leading-tight">
                  {mod.title}
                  <span className="block text-lg sm:text-xl text-gray-500 font-semibold mt-1">
                    {mod.subtitle}
                  </span>
                </h1>
              </div>
            ...
            <div className="flex flex-wrap gap-3 mb-8">
              <span className="text-xs px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-gray-400">
                ⏱️ {mod.estimatedHours}
              </span>
              <span className="text-xs px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-gray-400">
                📊 {mod.difficulty}
              </span>
              <span className="text-xs px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-gray-400">
                🧩 {mod.codeBlocks.length} Code Blocks
              </span>
              <span className="text-xs px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-gray-400">
                🏗️ 1 Project
              </span>
            ...
            {/* Objectives */}
            <div className="rounded-2xl border border-white/8 bg-[#0e0e12] p-6 mod-fade-up">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* ── Prerequisites ───────────────────────────── */}
          <section className="mb-12 mod-fade-up">
            <div className="rounded-2xl border border-white/8 bg-[#0e0e12] p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {mod.prerequisites.map((p) => (
                  <span key={p} className="text-xs px-3 py-1.5 rounded-lg bg-purple-500/10 border border-purple-500/20 text-purple-300">
                    {p}
                  </span>
                ))}
              </div>
          ...
          <section className="mb-16">
            <div className="flex items-center gap-3 mb-8 mod-fade-up">
            ...
            <div className="space-y-6">
              {mod.theorySections.map((section, idx) => (
                <div
                  key={idx}
                  className="theory-card rounded-2xl border border-white/8 bg-[#0e0e12] p-6 sm:p-8 mod-fade-up"
                >
                        ...
                        <thead>
                          <tr>
                            {section.table.headers.map((h) => (
                              <th
                                key={h}
                                className="text-left text-xs font-bold text-gray-400 uppercase tracking-wider px-4 py-2 border-b border-white/10"
                              >
                                {h}
                              </th>
                            ))}
          ...
          <section className="mb-16">
            <div className="flex items-center gap-3 mb-8 mod-fade-up">
              <div className={`w-1 h-8 rounded-full bg-gradient-to-b ${gradient}`} />
              <h2 className="text-2xl font-black text-white">💻 Code Implementation</h2>
            </div>

            <div className="space-y-10">
              {mod.codeBlocks.map((block) => (
                <div key={block.id} className="mod-fade-up">
          ...
          <section className="mb-16">
            <div className="flex items-center gap-3 mb-8 mod-fade-up">
              <div className={`w-1 h-8 rounded-full bg-gradient-to-b ${gradient}`} />
              <h2 className="text-2xl font-black text-white">🏗️ Practical Project</h2>
            </div>

            <div className="rounded-2xl p-[1px] bg-gradient-to-r from-purple-500 via-pink-500 to-purple-500 mod-fade-up">
              <div className="rounded-2xl bg-[#0d0d10] p-6 sm:p-8">
                ...
                <CodeBlock
                  code={mod.project.code.code}
                  language={mod.project.code.language}
          ...
          <section className="mb-12 mod-fade-up">
            ...
            <div className="flex flex-wrap gap-2">
              {mod.tags.map((tag) => (
                <span
                  key={tag}
                  className="text-xs px-3 py-1.5 rounded-lg bg-white/5 border border-white/8 text-gray-400"
                >
                  #{tag}
                </span>
              ))}
